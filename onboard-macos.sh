...
- Claude Code CLI(就是現在這個 `claude` 終端機工具):跑在 terminal 裡,能直接讀寫專案檔案、執行指令、跑 git,是真正「動手做事」的 agent。
...
A. 用 gh CLI(終端機):先 `gh auth status` 確認已登入(沒登入就帶我 `gh auth login`),再:
