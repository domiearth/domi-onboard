...
```bash
curl -fsSL https://raw.githubusercontent.com/domiearth/domi-onboard/main/onboard-macos.sh -o ~/onboard-macos.sh && \
bash ~/onboard-macos.sh -t <GitHub PAT> -h 192.168.0.141 --host-tailscale 100.72.24.53 -u domi -p '<hub 密碼>'
```
...
- **新人導覽** — 最後一步(Step 11)會問要不要開導覽,**按 Enter 就對了**,Claude 會用中文一步一步帶你跑過工具與流程(自動開始;沒開始就在 session 內打 `/guide`)。跳過也沒關係,見下方。
